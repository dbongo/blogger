...
trait Security { self: Controller =>
  ...
  def HasToken[A](p: BodyParser[A] = parse.anyContent)(
    ...
    Action(p) { implicit request => request.cookies.get("XSRF-TOKEN").fold {
      Unauthorized(Json.obj("message" -> "Invalid XSRF Token cookie"))
    } { xsrfTokenCookie =>
      val maybeToken = request.headers.get(AuthTokenHeader).orElse(request.getQueryString(AuthTokenUrlKey))
        maybeToken flatMap { token =>
          Cache.getAs[String](token) map { userId =>
            if (xsrfTokenCookie.value.equals(token)) { f(token)(userId)(request)}
            else { Unauthorized(Json.obj("message" -> "Invalid Token"))}}
